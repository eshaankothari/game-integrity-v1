...
DIRS = {
    ...
    # LLM responses. Cached for the same reason the odds snapshots are: a re-run of a
    # 4,810-game pass would otherwise re-bill every call. The key carries a hash of the
    # PROMPT and the MODEL, so changing either is a cache miss rather than a stale hit.
    "llm":      (None,                  config.LLM_CACHE_V1),
}
...
def llm_key(task, model, prompt_text, ident):
    """'review_claude-haiku-4-5_a1b2c3d4_1629007_0022300999.json'

    The prompt HASH is in the key on purpose. An LLM answer is only reusable if the
    question was identical, so editing the prompt must miss the cache rather than
    silently serve an answer to the old question -- which is the LLM equivalent of the
    stale-column bug this project has hit three times.
    """
    import hashlib
    h = hashlib.sha256(prompt_text.encode()).hexdigest()[:8]
    return f"{task}_{model}_{h}_{ident}.json"
