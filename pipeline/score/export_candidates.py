...
# Cut 7. Seasons of NBA experience PRIOR to 2023-24, from the roster endpoint ('R' -> 0).
# `experience <= MIN_EXP` is removed, so 2 keeps players in their fourth season or later.
MIN_EXP = 2
...
SQL = """
SELECT p.full_name AS player, g.matchup, p.experience, f.*,
...
EXPORT = ["rank", "rank_all", "player", "game_date", "matchup", "tier",
          ...
          "close_under", "open_line", "open_under", "open_source",
          "open_offset_hours", "line_move_pct",
          "under_move_pct", "price_only_move", "p_price", "p_line",
          "salary", "has_listed_salary", "experience", "under_hit",
...
def main():
    ...
    cuts = [
        ...
        # `~(x <= MIN_EXP)` rather than `x > MIN_EXP`, so the 12 propped players the
        # roster endpoint returned no EXP for are KEPT, not silently deleted. Same rule
        # as cuts 4-6: a row with nothing to test is not a row that failed the test.
        # Jontay Porter is one of those 12, so this branch is load-bearing -- the
        # written form would remove him and the cut would delete a labelled case.
        (f"7  experience > {MIN_EXP} yrs  (NaN kept)", ~(d["experience"] <= MIN_EXP)),
    ]
