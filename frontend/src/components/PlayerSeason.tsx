...
import { RED_SCORE, scoreLevel, fmtScore, isConfirmed } from "../severity";
    ...
    <div className="block">
      <h3 className="ps-title">
        ...
        <span className="ps-sub">
          {rows.length} scored game{rows.length === 1 ? "" : "s"}
        </span>
      </h3>
          ...
          <div className="cal-week" key={wi}>
            {week.map((day) => {
              ...
              return (
                <motion.button
                  key={iso(day)}
                  className={`cal-cell mini ${r ? scoreLevel(r.score_100) : "lv0"} ${isCur ? "cur" : ""}`}
                  disabled={!r}
                  title={r ? `${r.game_date} · ${r.points} pts / ${r.line} · ${fmtScore(r.score_100)}${isConfirmed(r.player_id, r.game_id) ? " · CONFIRMED" : ""}` : undefined}
            ...
            return (
              ...
              >
                <td>
                  ...
                  {isConfirmed(r.player_id, r.game_id) && (
                    <span className="conf-tag">CONFIRMED</span>
                  )}
                </td>
                <td className={r.score_100 >= RED_SCORE ? "hot" : ""}>
                  {fmtScore(r.score_100)}
                </td>
              </motion.tr>
            );
          })}
      ...
      <div className="blocknote">
        Displaying top games based on suspicion score
      </div>
    </div>
  );
}
